...
export class CommonService {
    ...
    async applyCursorPaginationParamsToQb<T extends ObjectLiteral>(
        ...
        if (cursor) {
            ...
            for (let i = 0; i < order.length; i++) {
                ...
                conditions.push(`${qb.alias}.${column} ${comparisonOperator} :${qb.alias}_${column}`);
    ...
    parseCursorPagination(dto: cursorPaginationDto) {
        let { cursor, take, order } = dto;
        let cursorWhere: Record<string, unknown> | undefined;

        if (cursor) {
            const decodeCursor = Buffer.from(cursor, 'base64').toString('utf-8');
            const cursorObj = JSON.parse(decodeCursor) as {
                values: Record<string, unknown>;
                order: string[];
            };

            order = cursorObj.order;
            cursorWhere = this.buildPrismaCursorWhere(order, cursorObj.values);
        }

        const orderBy = order.map((columnOrder) => {
            ...
            return { [column]: direction.toLowerCase() as 'asc' | 'desc' };
        });

        return { cursorWhere, orderBy, take, order };
    }

    private buildPrismaCursorWhere(order: string[], values: Record<string, unknown>) {
        ...
        for (let i = 0; i < order.length; i++) {
            ...
            for (let j = 0; j < i; j++) {
                const [previousColumn] = order[j].split('_');
                andConditions.push({ [previousColumn]: values[previousColumn] });
            }

            andConditions.push({ [column]: { [operator]: values[column] } });
            orConditions.push(andConditions.length === 1 ? andConditions[0] : { AND: andConditions });
        }
